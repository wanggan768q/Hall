...
public class DataEncrypt : MonoBehaviour
{
    private static DataEncrypt _MyDataEncrypt;
    ...
    {
        char[] chArray = RSAKey.ToCharArray();
        int index = 1;
        for (int i = chArray.Length - 2; index < i; i--)
        {
    ...
    public static DataEncrypt GetSingleton()
    {
    ...
    public long GetUnixTime()
    {
        ...
        num = (long) Math.Round(span.TotalMilliseconds, MidpointRounding.AwayFromZero);
        return (this.serverTime + (num - this.getServerTime));
    }
    ...
    {
        this.m_publicKey = "none";
        this.m_privateKey = "none";
        string[] strArray = new string[2];
        strArray = this.RSAKey();
        this.m_privateKey = strArray[0];
        this.m_publicKey = strArray[1];
        XmlDocument document = new XmlDocument();
        document.LoadXml(this.m_publicKey);
        XmlNodeList list = document.SelectNodes("/RSAKeyValue");
        string innerText = list[0].SelectSingleNode("//Modulus").InnerText;
        string str2 = list[0].SelectSingleNode("//Exponent").InnerText;
        return new string[] { this.EncryptRSAKey(innerText), str2 };
    }

    public string RSADecrypt(string content)
    {
        if (this.m_privateKey == "none")
        {
            return null;
        }
        string privateKey = this.m_privateKey;
        RSACryptoServiceProvider provider = new RSACryptoServiceProvider();
        provider.FromXmlString(privateKey);
        byte[] bytes = provider.Decrypt(Convert.FromBase64String(content), false);
        return Encoding.UTF8.GetString(bytes);
    }

    public string RSAEncrypt(string content)
    {
        if (this.m_publicKey == "none")
        {
            return null;
        }
        string publicKey = this.m_publicKey;
        RSACryptoServiceProvider provider = new RSACryptoServiceProvider();
    ...
    {
        int index = new System.Random().Next(0, 30);
        if (index < 0)
        {
            index = 0;
        }
        ...
        return new string[] { this.PrivateKeyPool[index], this.PublicKeyPool[index] };
    }

    public void setServerTime(long setTime)
    {
        this.serverTime = 0L;
        this.getServerTime = 0L;
        DateTime time = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(0x7b2, 1, 1, 0, 0, 0, 0));
        DateTime now = DateTime.Now;
        this.serverTime = setTime;
        TimeSpan span = (TimeSpan) (now - time);
        this.getServerTime = (long) Math.Round(span.TotalMilliseconds, MidpointRounding.AwayFromZero);
    }
}
